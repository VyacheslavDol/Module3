...
{      
    int read_number;
    struct sembuf connect = {0, -1, 0};
    struct sembuf disconnect = {0, 1, 0};
    struct sembuf wait_all = {2, 0, 0};
    struct sembuf wait_0 = {1, 0, 0};       //"ожидание нуля" для 1 семафора
    struct sembuf alrd_read = {2, -1, 0};

    //открытие файла на чтение
    int fd_read = open(FNAME, O_RDONLY | O_CREAT);        
    if (fd_read == -1) {
...
int main(int argc, char* argv[])
{
    ...
    sem = semget(key, 3, IPC_CREAT | 0666);         // 0 - кол-во процессов которые читают файл в данный момент времени; 1 - доступ (0/1); 2 - кол-во процессов поричтавших файл
    if (sem == -1)                              // 1 - доступ к чтению (0/1); 2 - кол-во процессов поричтавших файл
    {
        perror("semget");
        exit(1);
    }
